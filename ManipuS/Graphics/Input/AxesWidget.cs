...
namespace Graphics
{
    public class AxesWidget
    {
        public class Axis
        {
            ...
            internal Vector3 Poll(Camera camera, ref Matrix4 view, ref Matrix4 proj, MouseState stateCurr)  // TODO: optimize
            {
                if (stateCurr.LeftButton == ButtonState.Pressed)
                {
                    ...
                    var planeNormal = Vector3.Cross(axisViewOrtho, InputHandler.RaycastResult.Direction.Xyz);

                    // find the point of intersection between the ray plane and the axis
                    var intersection = LinePlaneIntersection(Start.Xyz, Direction, InputHandler.RaycastResult.StartWorld.Xyz, planeNormal);
            ...
            internal bool IsActive(ref Matrix4 view, ref Matrix4 proj, out Vector3 endNDC)
            {
        ...
        public void Poll(Camera camera, MouseState mouseState)
        {
            ...
            if (ActiveAxis != null)
            {
                ...
                var translation = ActiveAxis.Poll(camera, ref view, ref proj, mouseState);
